...
class MySessionInterface(SessionInterface):
    ...
    def open_session(self, app, request):
        sessionId = request.cookie.get(app.session_cookie_name)
        if not sessionId:
        ...
        try:
            unsignedSessionId = signer.unsign(sessionId).decode()
        except BadSignature:
            sessionId = str(uuid.uuid4())
            return self.session_class(sessionId=sessionId)
    ...
    def save_session(self, app, session, response):
        ...
        signer = Signer(app.secret_key, salt=self.salt, key_derivation="hmac")
        signedId = signer.sign(want_bytes(session.sessionId))
        response.set_cookie(app.session_cookie_name, signedId)
